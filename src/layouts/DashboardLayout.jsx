  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="no-print fixed inset-x-0 top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white px-4 lg:px-7">
        ...
        <div className="ml-auto"><HeaderUserMenu /></div>
      </header>
      <aside className={`no-print fixed bottom-0 left-0 top-16 z-20 hidden border-r border-slate-200 bg-white transition-[width] duration-150 lg:block ${collapsed ? "w-[72px] p-3" : "w-60 p-4"}`}>
      ...
      <main className={`min-w-0 overflow-x-hidden pt-16 transition-[padding] duration-150 ${collapsed ? "lg:pl-[72px]" : "lg:pl-60"}`}><Outlet /></main>
    </div>
  );
}
